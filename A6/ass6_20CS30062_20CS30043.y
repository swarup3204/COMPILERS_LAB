%{
    ...
    extern quad_array quadList;              // List of all quads
    extern symbol_table global_symbol_table;            // Global symbol table
    extern symbol_table* ST;                 // Pointer to the current symbol table
...
%union {
    ...
    symbol_type* symType;            // For the type of a symbol
...
primary_expression: 
        ...
        | INTEGER_CONSTANT
        {
            ...
            val->set_initial_value($1);                    // Set the initial value
...
postfix_expression: 
        ...
        {
            symbol_type to = ST->lookup($1->loc)->type;      // Get the type of the expression
            string f = "";
            if(!($1->fold)) {
        ...
        {   
            $$ = new expression();                                          // Create new expression
            symbol_type t = ST->lookup($1->loc)->type;                       // Get the type of the expression and generate a temporary variable
            if(t.type == ARRAY) {
        ...
        | postfix_expression DECREMENT
        {
            ...
            symbol_type t = ST->lookup($1->loc)->type;
            if(t.type == ARRAY) {
                $$->loc = ST->gentemp(ST->lookup($1->loc)->type.nextType);
                string temp = ST->gentemp(t.nextType);
                emit(temp, $1->loc, *($1->folder), ARR_IDX_ARG);
...
unary_expression: 
        ...
        {
            $$ = new expression();
            symbol_type type = ST->lookup($2->loc)->type;
            if(type.type == ARRAY) {
...
multiplicative_expression: 
        cast_expression
        {
            ...
            symbol_type tp = ST->lookup($1->loc)->type;
...
function_definition: 
        ...
        {
            ST = &global_symbol_table;                     // Reset the symbol table to global symbol table
